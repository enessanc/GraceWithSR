    ...
    def forward(self, sr, hr):
        ...
        try:
            with torch.amp.autocast(device_type='cuda'):
                ...
                # Loss'u anlamlı değerlere ölçeklendir
                loss = loss * 0.01
...
class CombinedLoss(nn.Module):
    def __init__(self, alpha=0.8, beta=0.15, gamma=0.05):
        ...
        self.beta = beta    # Perceptual loss ağırlığı
    ...
    def forward(self, sr, hr):
        ...
        try:
            ...
            return total_loss, l1, perceptual, ssim_loss

        except Exception as e:
            return torch.tensor(0.5, device=sr.device), torch.tensor(0.5, device=sr.device), torch.tensor(0.5, device=sr.device), torch.tensor(0.5, device=sr.device)

from .sr4_loss import calculate_ssim_loss, PerceptualLoss, CombinedLoss

__all__ = ['calculate_ssim_loss', 'PerceptualLoss', 'CombinedLoss']
